...
$preimprime         = CValue::get("preimpression", 0); //A mettre surement en config
...
if($edit_bvr){
  $acompte = $facture->_reglements_total_patient+0;
  $nb_factures = count($facture->_montant_factures);
  
	foreach($facture->_montant_factures as $num_fact => $montant_facture){
      ...
      if($num_fact == 0){
        $montant_facture = sprintf('%0.2f', $montant_facture - $facture->_reglements_total_patient);
      }
      else{
        $montant_facture = sprintf('%0.2f', $montant_facture);
		  ...
		  $tarif = array(
		    ...
		    "Medical:"      => $pm,
		    "Technique:"    => "$pt",
        "Remise:"       => "- ".sprintf('%0.2f', $facture->remise),
		    "Montant total:" => sprintf('%0.2f', $facture->_montant_avec_remise),
		    "Acompte:"      => sprintf('%0.2f', $acompte),
		    "Montant dû $title_montant:"   => $montant_facture
		  );
		  $acompte += $montant_facture;
		  $pdf->Line($colonne2, 122, $colonne2+50, 122);
		  $pdf->Line($colonne2, 133, $colonne2+50, 133);
	  ...
	  if(!$preimprime){
		...
		for($i = 0; $i<=1; $i++){
		  ...
		  if($factureconsult_id){
		  	$pdf->setFont($font, '', 10);
		  	$pdf->SetTextColor(0);
				$pdf->Text($l_colonne*(19-strlen($montant_facture*100)-2) + $decalage, $h_ligne*13+$haut_doc , sprintf("%d", $montant_facture));
				
				$cents = ceil(($montant_facture - sprintf("%d", $montant_facture))*100);
				if($cents<10){			
					$cents = "0".$cents;
				}
