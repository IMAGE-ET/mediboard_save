<form name="edit-CDailyCheckList-{{$check_list_group->_guid}}" method="post" action="?"
  ...
  <table class="main form">
    {{mb_include module=system template=inc_form_table_header object=$check_list_group}}
    {{if $duplicate}}
      <tr>
    ...
    <tr>
      <td class="button" colspan="2">
        {{if $duplicate && !$check_list_group->_id}}
          <button class="duplicate" type="button" onclick="return onSubmitFormAjax(this.form, {onComplete: function() {location.reload()}});">
            {{tr}}Duplicate{{/tr}}
          </button>
          <button class="cancel" type="button" onclick="CheckListGroup.modal.close();">{{tr}}Cancel{{/tr}}</button>
        {{else}}
...
{{if $check_list_group->_id}}
  <table class="main tbl">
    <tr>
      <td colspan="3">
        <button type="button" class="add" onclick="CheckListGroup.editChecklist('0', '{{$check_list_group->_id}}')">{{tr}}CDailyCheckListType-title-create{{/tr}}</button>
      </td>
    </tr>
    <tr>
    ...
    {{foreach from=$check_list_group->_ref_check_liste_types item=_list}}
      <tr>
        <td>
          <button type="button" class="edit notext" onclick="CheckListGroup.editChecklist('{{$_list->_id}}')"></button>
