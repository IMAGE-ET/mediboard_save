...
if (class_exists('Squiz_Sniffs_ControlStructures_SwitchDeclarationSniff', true) === false) {
  throw new PHP_CodeSniffer_Exception('Class Squiz_Sniffs_ControlStructures_SwitchDeclarationSniff not found');
}

class Standard_Sniffs_ControlStructures_SwitchDeclarationSniff extends Squiz_Sniffs_ControlStructures_SwitchDeclarationSniff {
  public $indent = 2; // CHANGED HERE FOR MB
  public $indentBreak = 4; // CHANGED HERE FOR MB
  ...
  public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
  {
    ...
    $caseAlignmentBreak = ($switch['column'] + $this->indentBreak); // CHANGED HERE FOR MB
    $caseCount     = 0;
    $foundDefault  = false;
    ...
    while (($nextCase = $phpcsFile->findNext(array(T_CASE, T_DEFAULT, T_SWITCH), ($nextCase + 1), $switch['scope_closer'])) !== false) {
      ...
      if ($tokens[$nextBreak]['code'] === T_BREAK
        ...
        if ($tokens[$nextBreak]['scope_condition'] === $nextCase) {
          ...
          // the default case.
          if ($tokens[$nextBreak]['column'] !== $caseAlignmentBreak) {
            $error = 'Case breaking statement must be indented '.$this->indentBreak.' spaces from SWITCH keyword'; // CHANGED HERE FOR MB
