       ...
       foreach($lines_by_type as $_line_med){
          if($_code_cip && ($_line_med->code_cip != $_code_cip)){
            continue;
	 				...
	 				if(!isset($besoin_patient[$_line_med->code_cip][$patient->_id])){
		        $besoins =& $besoin_patient[$_line_med->code_cip][$patient->_id];
			      ...
			      foreach($_prises_by_hour as $_hour => $_prise){
			        foreach($_perfusion->_ref_lines as $_perf_line){
			          ...
			          if($datetime < "$date_min_orig 00:00:00" || $datetime > "$date_max_orig 23:59:59"){
			        	...
			        	if(!isset($besoin_patient[$_perf_line->code_cip][$patient->_id])){
					        $besoins =& $besoin_patient[$_perf_line->code_cip][$patient->_id];
			            $besoins = array("patient" => "", "quantite_administration" => 0, "quantite_dispensation" => 0);
					      }
