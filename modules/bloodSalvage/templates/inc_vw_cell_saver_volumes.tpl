...
<table class="form">
  <tr>
	...
	<tr>
	  <th>{{mb_label object=$blood_salvage field=wash_volume}}</th>
		<td>
      {{mb_field object=$blood_salvage field=wash_volume size=5}}      ml
		  <button type="button" class="tick notext" onclick="submitFormAjax(this.form, 'systemMsg');">{{tr}}Save{{/tr}}</button>
		...
		<td>
      {{mb_field object=$blood_salvage field=hgb_pocket size=5}}		  g/dl
		  <button type="button" class="tick notext" onclick="submitFormAjax(this.form, 'systemMsg');">{{tr}}Save{{/tr}}</button>  
	...
	<tr>
	  <th>{{mb_label object=$blood_salvage field=saved_volume}}</th>
		<td>
      {{mb_field object=$blood_salvage field=saved_volume size=5}}		   ml
		  <button type="button" class="tick notext" onclick="submitFormAjax(this.form, 'systemMsg');">{{tr}}Save{{/tr}}</button>	   
	  ...
	  <th>{{mb_label object=$blood_salvage field=hgb_patient}}</th>	
		<td>
      {{mb_field object=$blood_salvage field=hgb_patient size=5}}     g/dl
		  <button type="button" class="tick notext" onclick="submitFormAjax(this.form, 'systemMsg');">{{tr}}Save{{/tr}}</button>  
  ...
  <th>{{mb_label object=$blood_salvage field=transfused_volume}}</th>
	<td colspan="4">
      {{mb_field object=$blood_salvage field=transfused_volume size=5}}      ml
      <button type="button" class="tick notext" onclick="submitFormAjax(this.form, 'systemMsg');">{{tr}}Save{{/tr}}</button>  
