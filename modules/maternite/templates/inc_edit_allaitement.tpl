  ...
  <table class="form">
      ...
      <td>
        <select name="grossesse_id">
          ...
          {{foreach from=$grossesses item=_grossesse}}
            <option value="{{$_grossesse->_id}}" {{if $_grossesse->_id == $allaitement->grossesse_id}}selected{{/if}}>{{$_grossesse}}</option>
          {{/foreach}}
        </select>
      </td>
    </tr>
    <tr>
      <td colspan="2" class="button">
        <button type="button" class="save" onclick="this.form.onsubmit()">{{if $allaitement->_id}}{{tr}}Save{{/tr}}{{else}}{{tr}}Create{{/tr}}{{/if}}</button>
