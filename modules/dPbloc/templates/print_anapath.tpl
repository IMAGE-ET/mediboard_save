...
<table class="tbl">
  ...
  <tr>
    ...
    <th>{{mb_title class=COperation field=labo_anapath}}</th>
  </tr>
  {{foreach from=$operations item=_op}}
  <tr>
    <td>{{mb_value object=$_op field=_datetime_best}}</td>
    <td>{{mb_include module=planningOp template=inc_vw_operation _operation=$_op}}</td>
    <td>{{mb_include module=system template=inc_vw_mbobject object=$_op->_ref_sejour->_ref_patient}}</td>
    <td>{{mb_include module=mediusers template=inc_vw_mediuser mediuser=$_op->_ref_chir}}</td>
    <td>{{mb_include module=system template=inc_vw_mbobject object=$_op->_ref_salle}}</td>
    <td class="button">{{mb_value object=$_op field=flacons_anapath}}</td>
    <td>{{mb_value object=$_op field=description_anapath}}</td>
    <td>{{mb_value object=$_op field=labo_anapath}}</td>
  </tr>
  {{foreachelse}}
  <tr>
