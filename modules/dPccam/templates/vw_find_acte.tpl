...
<div id="multiple" class="reseach">
  <form id="multiple_form" method="get" onsubmit="return CCodageCCAM.submitFunction(this);">
      ...
      <tr>
        ...
        <td>
          <table class="main form">
              ...
              <td>
                <select name="chap1" id="chap1" style="width:50%;"
                  ...
                  {{foreach from=$listChap1 item=curr_chap key=key_chap}}
                    <option value="{{$curr_chap.rang}}" data-code-pere="{{$key_chap}}">
                      {{$curr_chap.rang}} - {{$curr_chap.texte|utf8_decode|lower}}
                    </option>
                  {{/foreach}}
                </select>
              </td>
           ...
           <!--   <th>Date :</th>
              <td>
                <input class="date notNull" type="hidden" name="date_demandee" value="" onchange="CCodageCCAM.cacheElements();" />
                <script type="text/javascript">
                  Main.add(function () {
                    Calendar.regField(getForm('multiple_form').date_demandee);
                  });
                </script>
              </td>-->
              <td></td>
