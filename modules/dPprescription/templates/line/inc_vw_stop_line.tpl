...
<form name="form-stop-{{$object_class}}-{{$line->_id}}" method="post" action="">
  ...
  {{if $line->date_arret}}
    <input type="hidden" name="date_arret" value="{{$line->date_arret}}" />
	  <button type="button" class="cancel" onclick="this.form.date_arret.value = ''; changeColor('{{$line->_id}}','{{$line->_class_name}}',this.form.date_arret.value,'{{$line->_traitement}}'); Prescription.submitFormStop(this.form,'{{$line->_id}}','{{$line->_class_name}}');">
	    Annuler l'arrêt
	  </button>
      ...
      <tr>
        <td class="date" style="border:none;">
          {{assign var=line_id value=$line->_id}}
	        {{mb_field object=$line field=date_arret form=stop-$object_class-$line_id}}
	        <button type="button" class="stop" onclick="calculDateArret(this.form, '{{$line->_id}}','{{$line->_class_name}}','{{$line->_traitement}}');">Arrêter</button>
	      </td>
	    </tr>
	  </table>
  {{/if}}
