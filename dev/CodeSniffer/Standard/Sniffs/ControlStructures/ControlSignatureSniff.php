    ...
    if ($patternInfo['listen_pos'] > 0) {
      ...
      for ($i = ($patternInfo['listen_pos'] - 1); $i >= 0; $i--) {
        if ($pattern[$i]['type'] === 'token') {
          ...
          if (
              $this->ignoreComments === true && in_array($tokens[$stackPtr]['code'], PHP_CodeSniffer_Tokens::$commentTokens) === true
          ) {
          ...
          if ($tokens[$stackPtr]['code'] === T_WHITESPACE) {
            if ($tokens[$stackPtr]['content'] !== $phpcsFile->eolChar) {
              ...
              $cont = $tokens[($stackPtr - 1)]['content'];
              $cont = preg_replace("@(//[^\r\n]+[\r\n]+)@", "", $cont); // ADDED
              if ($cont !== "") {
                if ($cont !== $phpcsFile->eolChar) {
                  $hasError = true;
                }
                else {
                  $stackPtr--;
