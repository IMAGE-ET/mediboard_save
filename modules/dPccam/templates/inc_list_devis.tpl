...
<table class="tbl">
  ...
  {{foreach from=$list_devis item=_devis}}
    <tr>
      ...
      <td class="narrow">
        <button class="edit notext" type="button" onclick="DevisCodage.edit('{{$_devis->_id}}', '{{$object->_class}}', '{{$object->_id}}');">
        ...
        <form name="deleteDevis-{{$_devis->_id}}" action="?" method="post" onsubmit="return onSubmitFormAjax(this, function() {
          ...
          <button class="trash notext" type="submit" onclick="" {{if $_devis->_count_actes != 0}} disabled="disabled"{{/if}}>
            {{tr}}Delete{{/tr}}
          </button>
        </form>
      </td>
