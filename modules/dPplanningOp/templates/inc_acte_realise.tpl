{{if $codable->_ref_actes|@count}}
  {{foreach from=$codable->_ref_actes item="acte" name="tab_codable"}}
    {{if $acte->executant_id == $praticien->_id}}
      {{if !$smarty.foreach.tab_codable.first}}
        <tr>
      {{/if}}
        <td>
      ...
      {{if $smarty.foreach.tab_codable.first && "dPfacturation"|module_active && $conf.dPplanningOp.CFactureEtablissement.use_facture_etab}}
      ...
      {{if !$smarty.foreach.tab_codable.last}}
        </tr>
      {{/if}}
    {{/if}}
  {{/foreach}}
