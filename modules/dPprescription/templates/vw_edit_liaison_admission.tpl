  ...
  <tr>
    ...
    <td>
      ...
      <form name="selProtocoleAnesth-{{$_protocole->_id}}" action="?" method="post">  
        ...
        {{foreach from=$protocoles_list_praticien.anesth item=_protocole_praticien}}
          <option value="{{$_protocole_praticien->_id}}" {{if !$is_admin && ($_protocole_praticien->_id == $_protocole->protocole_prescription_anesth_id)}}selected="selected"{{/if}}>{{$_protocole_praticien->_view}}</option>
        {{/foreach}}
        {{foreach from=$protocoles_list_function.anesth item=_protocole_function}}
          <option value="{{$_protocole_function->_id}}" {{if !$is_admin && ($_protocole_function->_id == $_protocole->protocole_prescription_anesth_id)}}selected="selected"{{/if}}>{{$_protocole_function->_view}}</option>
        {{/foreach}}
        </select>
      </form>
      {{if $is_admin}}
    ...
    <td>  
      ...
      <form name="selProtocoleChir-{{$_protocole->_id}}" action="?" method="post">  
        ...
        {{foreach from=$protocoles_list_praticien.chir item=_protocole_praticien}}
          <option value="{{$_protocole_praticien->_id}}" {{if $_protocole_praticien->_id == $_protocole->protocole_prescription_chir_id}}selected="selected"{{/if}}>{{$_protocole_praticien->_view}}</option>
        {{/foreach}}
        {{foreach from=$protocoles_list_function.chir item=_protocole_function}}
          <option value="{{$_protocole_function->_id}}" {{if $_protocole_function->_id == $_protocole->protocole_prescription_chir_id}}selected="selected"{{/if}}>{{$_protocole_function->_view}}</option>
        {{/foreach}}
        </select>
      </form>
