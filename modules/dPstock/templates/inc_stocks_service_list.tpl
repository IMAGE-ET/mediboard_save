...
<table class="tbl">
  <tr>
    ...
    <th>{{tr}}CProductStockService-function_id{{/tr}}</th>
...
{{foreach from=$list_stocks_service item=curr_stock_service}}
  <tr>
    <td><a href="?m={{$m}}&amp;tab=vw_idx_stock_service&amp;stock_id={{$curr_stock_service->_id}}" title="{{tr}}CProductStockService.modify{{/tr}}">{{$curr_stock_service->_ref_product->_view}}</a></td>
    <td>{{$curr_stock_service->_ref_function->_view}}</td>
