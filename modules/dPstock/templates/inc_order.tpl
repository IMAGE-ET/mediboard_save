  ...
  <tr>
    <td colspan="8" id="order-{{$order->_id}}-total" style="border-top: 1px solid #666;">
      ...
      <form name="order-lock-{{$order->_id}}" action="?" method="post">
        ...
        <button type="button" class="tick" onclick="submitOrder(this.form, {close: false, confirm: true, onComplete: function(){refreshLists()}});">{{tr}}CProductOrder-_validate{{/tr}}</button>
      </form>
      {{/if}}
    </td>
  </tr>
