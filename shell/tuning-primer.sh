...
## -- Key buffer Size -- ##
	...
	if [ -z "$myisam_indexes" ] ; then
		myisam_indexes=`find $datadir -name '*.MYI' -exec du $duflags '{}' \; | awk '{ s += $1 } END { printf("%i\n", s )}'`
	fi
...
## XXX a column of numbers... | awk '{ s += $2 } END { print s }' XXX ##
