...
<form name="param-merge-matching-patients" method="get" action="?" onsubmit="return Url.update(this, 'matching-patients-messages')">
  ...
  <fieldset>
    ...
    <label><input type="checkbox" name="naissance[day]"   value="1" {{if @$naissance.day}}   checked="checked" {{/if}} /> {{tr}}Day{{/tr}}</label>
    <label><input type="checkbox" name="naissance[month]" value="1" {{if @$naissance.month}} checked="checked" {{/if}} /> {{tr}}Month{{/tr}}</label>
    <label><input type="checkbox" name="naissance[year]"  value="1" {{if @$naissance.year}}  checked="checked" {{/if}} /> {{tr}}Year{{/tr}}</label>
  </fieldset>
