  ...
  <table class="tbl" style="max-height: 400px; overflow-y: auto;">
    ...
    {{foreach from=$object->_ref_operations item=_op}}
      ...
      <tr>
        ...
        <td class="{{$class}}"><span onmouseover="ObjectTooltip.createEx(this, '{{$_op->_ref_patient->_guid}}');">
            {{$_op->_ref_patient}}
          </span></td>
        <td class="{{$class}} compact text">{{$_op->libelle}}</td>
        <td class="{{$class}}">{{$_op->_time_op|date_format:$conf.time}}</td>
        <td class="{{$class}}">{{if $_op->fin_op }}<img src="images/icons/tick.png" alt="" style="height: 30px;"/> {{/if}}</td>
      </tr>
      {{foreachelse}}
      <tr>
