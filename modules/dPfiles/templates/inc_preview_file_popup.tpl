<table class="form">
  <tr>
    ...
    <td class="button" style="width:60%">
      ...
      Catégorie : {{if $listCat->nom}}{{$listCat->nom}}{{else}}Aucune Catégorie{{/if}} &mdash; Date : {{$file->file_date|date_format:"%d/%m/%Y à %Hh%M"}}
     {{/if}}
    </td>
    <th style="width:20%">
    {{if $fileNext}}
    ...
    <td class="button" colspan="3">      
      {{if $file->file_id}}
        {{if $file->_nb_pages}}
            ...
            {{/if}}
        {{/if}}<br />
        <a href="mbfileviewer.php?file_id={{$file->file_id}}"><img src="mbfileviewer.php?file_id={{$file->file_id}}&amp;phpThumb=1&amp;w=700{{if $sfn}}&amp;sfn={{$sfn}}{{/if}}" alt="-" border="0" /></a>
      {{/if}}
    </td>
  </tr>
</table>