...
<form name="printFactures" action="" method="get">
  ...
  <table class="form main tbl">
    ...
    <tr>
      ...
      <th class="category" style="width:60px;">Montant</th>
    </tr>
    {{foreach from=$factures item=facture}}
      <tr>
        ...
        <td>{{mb_value object=$facture->_ref_last_sejour field=entree format="%d/%m/%Y"}}</td>
        <td>{{mb_value object=$facture->_ref_last_sejour field=sortie format="%d/%m/%Y"}}</td>
        <td>
          {{if $facture->envoi_xml}}
    ...
    {{/foreach}}
    <tr>
      <td colspan="8" style="text-align: right;"><strong>{{mb_label class=CFactureEtablissement field=_montant_total}}</strong></td>
      <td style="text-align: right;">{{$montant_total|string_format:"%0.2f"}}</td>
    </tr>
    <tr>
      <td colspan="9" class="button">
        <button class="print" type="button" onclick="impression();">Imprimer</button>
      </td>
    </tr>
  </table>
</form>