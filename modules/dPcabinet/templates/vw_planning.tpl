...
function showConsultations(oTd, plageconsult_id){
  oTd = $(oTd);
  if (oTd.parentNode.className != "selectedPlage hour_start") {
  	$$('td.selectedPlage.hour_start').each(function(elem){
  		elem.className = "nonEmpty hour_start";
  	});
  	
  	oTd.up().className = "selectedPlage hour_start";
...
<table class="main">
  ...
  <tr>
            ...
            {{foreach from=$listDays item=curr_day}}
              ...
              {{else}}
                ...
                <td class="{{if $plageconsult_id == $plage->plageconsult_id}}selectedPlage{{else}}nonEmpty{{/if}} {{if $curr_mins == '00'}}hour_start{{/if}}" rowspan="{{$plage->_nb_intervals}}">
                  <a href="#" onmouseover="showConsultations(this,'{{$plage->plageconsult_id}}');" onclick="PlageConsultation.edit('{{$plage->plageconsult_id}}');"  title="Voir le contenu de la plage">
    ...
    {{if $chirSel!=-1}}
    <a class="button new" onclick="PlageConsultation.edit('0');">Créer une nouvelle plage</a>
    {{/if}}
    </td>
    <td id="consultations">
