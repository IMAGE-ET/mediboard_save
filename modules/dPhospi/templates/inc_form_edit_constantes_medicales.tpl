...
<form name="edit-constantes-medicales" action="?" method="post" onsubmit="return {{if $real_context}}checkForm(this){{else}}false{{/if}}">
  ...
  <table class="main form" style="width: 1%;">
    ...
    <tr>
      ...
      {{if $real_context}}<td>{{mb_field object=$constantes field=_vst size="4" readonly="readonly"}}</td>{{/if}}
      <td>{{mb_value object=$const field=_vst}}{{if $const->_vst}}{{/if}}</td>
      <td />
    </tr>
    <tr>
      <th>{{mb_label object=$constantes field=_imc}}</th>
      {{if $real_context}}<td>{{mb_field object=$constantes field=_imc size="4" readonly="readonly"}}</td>{{/if}}
