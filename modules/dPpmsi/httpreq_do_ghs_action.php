...
switch ($type) {
  case "extractFiles": extractFiles();
    break;
  case "AddCM"       : addcm();
    break;
  case "AddDiagCM"   : adddiagcm();
    break;
  case "AddActes"    : addactes();
    break;
  case "AddGHM"      : addghm();
    break;
  case "AddCMA"      : addcma();
    break;
  case "AddIncomp"   : addincomp();
    break;
  case "AddArbre"    : addarbre();
    break;
 ...
 *
 * @see modules/dPpmsi/ghm/diagCM.txt
 * @return void
 */
function adddiagcm() {
  global $regCim10, $regCCAM, $alnum, $alpha, $targetDir;
...
) /*! ENGINE=MyISAM */ DEFAULT CHARSET=latin1 COMMENT='Table des diagnostics entree dans les CM';";
  $ds->exec($sql);
  if ($error = $ds->error()) {
    echo "<div class='error'>$error ($sql)</div>";
  }
  ...
  while (!feof($file)) {
    ...
    elseif (preg_match("`^($regCim10)`", $line, $diag)) {
 ...
 *
 * @see modules/dPpmsi/ghm/Actes.txt
 * @return void
 */
function addactes() {
  global $regCim10, $regCCAM, $alnum, $alpha, $targetDir;
  ...
  while (!feof($file) ) {
    ...
    elseif (preg_match("`^Liste ([AD]-[[:digit:]]*) : ([{$alnum}[:space:][:punct:]]*)`", $line, $liste) && $curr_cmd) {
      $curr_liste = $liste[1];
    ...
    elseif (preg_match("`^($regCCAM)/([[:digit:]])`", $line, $acte) && $curr_liste) {
    ...
    elseif (preg_match("`^($regCim10)`", $line, $diag) && $curr_liste) {
 ...
 *
 * @see modules/dPpmsi/ghm/GHM.txt
 * @return void
 */
function addghm() {
  global $regCim10, $regCCAM, $alnum, $alpha, $targetDir;
  ...
  while (!feof($file)) {
    ...
    elseif (preg_match("`^Groupes ([{$alnum}[:space:][:punct:]]*)`", $line, $groupe)) {
      $curr_group = $groupe[1];
      //echo "<div class='info'> Curr_groupe = $curr_groupe</div>";
    }
    elseif (preg_match("`^([[:digit:]]{2}[{$alpha}][[:digit:]]{2}[{$alpha}]) ([{$alnum}[:space:][:punct:]]*)`", $line, $GHM)) {
      $sql = "INSERT INTO ghm" .
 ...
 *
 * @see modules/dPpmsi/ghm/cma.txt
 * @see modules/dPpmsi/ghm/cmas.txt
 * @see modules/dPpmsi/ghm/cmasnt.txt
 * @return void
 */
function addcma() {
  global $regCim10, $regCCAM, $alnum, $alpha, $targetDir;
 ...
 *
 * @see modules/dPpmsi/ghm/incomp.txt
 * @return void
 */
function addincomp() {
  global $regCim10, $regCCAM, $alnum, $alpha, $targetDir;
 ...
 *
 * @see modules/dPpmsi/ghm/arbreGHM.csv
 * @return void
 */
