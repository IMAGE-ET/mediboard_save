{{if $line->_can_modify_comment}}
	...
	<input type="text" name="commentaire" size="60" value="{{$line->commentaire}}" 
         onchange="testPharma({{$line->_id}}); 
                   {{if $line->_class_name == 'CPrescriptionLineMedicament' && $line->substitute_for && !$line->substitution_active}}submitEditCommentaireSubst('{{$line->_id}}',this.value);{{else}}submitAddComment('{{$line->_class_name}}', '{{$line->_id}}', this.value);{{/if}}" />
{{else}}
  {{if $line->commentaire}}
