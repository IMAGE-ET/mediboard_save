...
foreach ($reglements as  $key => $_reglement) {
  ...
  if(CModule::getInstalled("tarmed") && CAppUI::conf("tarmed CCodeTarmed use_cotation_tarmed")){
	...
	if(CAppUI::conf("dPccam CCodeCCAM use_cotation_ccam")){
		if($_reglement->object_class == "CConsultation" && count($_reglement->_ref_object->_ref_actes_ngap) == 0 && count($_reglement->_ref_object->_ref_actes_ccam) == 0){
			$del = 1;
		}
...
$smarty->display("print_compta2.tpl");
