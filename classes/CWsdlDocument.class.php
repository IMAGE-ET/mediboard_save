...
class CWsdlDocument extends CMbXMLDocument {
  ...
  function __construct() {
    ...
    $definitions = $this->addElement($this, "wsdl:definitions", null, "http://schemas.xmlsoap.org/wsdl/");
  ...
  function addTypes() {
    ...
    $types = $this->addElement($definitions, "wsdl:types", null, "http://schemas.xmlsoap.org/wsdl/");
  ...
  function addMessage($functions, $returns = array()) {
    ...
    foreach($functions as $nameFunction => $atts) {
      $message = $this->addElement($definitions, "wsdl:message", null, "http://schemas.xmlsoap.org/wsdl/");
      $this->addAttribute($message, "name", $nameFunction."Request");
      
      foreach($atts as $attName => $attValue) {
        $part = $this->addElement($message, "wsdl:part", null, "http://schemas.xmlsoap.org/wsdl/");
  ...
  function addPortType($functions) {
    ...
    $portType = $this->addElement($definitions, "wsdl:portType", null, "http://schemas.xmlsoap.org/wsdl/");
    $this->addAttribute($portType, "name", "MediboardPort");
    
    foreach($functions as $nameFunction => $atts) {
      $partie5 = $this->createComment("partie 5 : Operation");
      $portType->appendChild($partie5);
      $operation = $this->addElement($portType, "wsdl:operation", null, "http://schemas.xmlsoap.org/wsdl/");
  ...
  function addBinding($functions) {
    ...
    $binding = $this->addElement($definitions, "wsdl:binding", null, "http://schemas.xmlsoap.org/wsdl/");
    ...
    foreach($functions as $nameFunction => $atts) {
      $operation = $this->addElement($binding, "wsdl:operation", null, "http://schemas.xmlsoap.org/wsdl/");
  ...
  function addService($username, $password, $module, $tab, $classname) {
    ...
    $service = $this->addElement($definitions, "wsdl:service", null, "http://schemas.xmlsoap.org/wsdl/");
    ...
    $port = $this->addElement($service, "wsdl:port", null, "http://schemas.xmlsoap.org/wsdl/");
