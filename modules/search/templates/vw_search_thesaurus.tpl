...
<table class="main layout">
  <tr>
    ...
    <td style="width: 30%">
      <form method="get" name="esSearchFavoris" action="?m=search" class="watched prepared"
            onsubmit="return Thesaurus.displayResultsThesaurus(this);"
            onchange="$V(this.form.start, '0'); $V(this.form.start_thesaurus, '0')">
    ...
    <td id="list_favoris">
      <fieldset>
        ...
        <form method="get" name="esFilterFavoris" action="?m=search" class="watched prepared"
              onsubmit="return Thesaurus.filterListThesaurus(this);" onchange="$V(this.form.start_thesaurus, '0')">
            ...
            <tr>
              <td class="halfPane">
                  <span class="circled">
                    ...
                    <label><input type="radio" name="_choose" value="user_id {{$app->_ref_user->_id}}" onclick="this.form.onsubmit();"
                                  onchange="$V(this.form.start_thesaurus, '0')" checked></label>
                  </span>

                  <span class="circled">
                     <img src="images/icons/user-function.png">
                      <label><input id="function" type="radio" name="_choose" value="function_id {{$user->_ref_function->_id}}"
                                    onclick="this.form.onsubmit();" onchange="$V(this.form.start_thesaurus, '0')"></label>
                  </span>

                  <span class="circled">
                    <img src="images/icons/group.png">
                    <label><input id="group" type="radio" name="_choose" value="group_id {{$g}}" onclick="this.form.onsubmit();"
                                  onchange="$V(this.form.start_thesaurus, '0')"></label>
                  </span>
              </td>
              <td>
