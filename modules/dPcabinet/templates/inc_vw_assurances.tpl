  ...
  <form name="assurances-patient" method="post" action="" > 
    ...
    <table class="main tbl">
      <tr>
        ...
        {{mb_include module=cabinet template="inc_vw_assurances_patient" object=$facture name="assurance_maladie"}}
        
        <td style="text-align:right;"> {{mb_label object=$facture field=assurance_accident}}</td>
        {{mb_include module=cabinet template="inc_vw_assurances_patient" object=$facture name="assurance_accident"}}
      </tr>
      <tr>
...
{{else}}
  <button type="button" class="add" onclick="Correspondant.edit(0, '{{$facture->_ref_patient->_id}}', refreshAssurance);">Ajouter des correspondants pour le patient</button>
{{/if}}