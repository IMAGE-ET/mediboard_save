...
class CFile extends CDocumentItem {
  ...
  function convertToPDF($file_path = null, $pdf_path = null) {
    ...
    // Requ�te post pour la conversion.
    // Cela permet de mettre un time limit afin de garder le contr�le de la conversion.
    ...
    // Fermeture de la session afin d'�crire dans le fichier de session
    session_write_close();
    
    // Le header Connection: close permet de forcer a couper la connexion lorsque la requ�te est effectu�e
    ...
    // La requ�te post r�ouvre la session
    ...
    // Si la conversion a �chou�,
    // on relance le service s'il ne r�pond plus.
    if ( $res != 1) {
