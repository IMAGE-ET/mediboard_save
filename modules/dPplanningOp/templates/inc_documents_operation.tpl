...
{{if @$preloaded}}
  ...
  <div class="documents-{{$object->_guid}} praticien-{{$object->chir_id}} mode-collapse" style="min-width: 200px; min-height: 50px; float: left; width: 50%;">
    {{mb_include module=compteRendu template=inc_widget_documents mode=collapse modelesByOwner=$modelesByOwner.COperation packsByOwner=$packsByOwner.COperation}}
  </div>
  ...
  <div class="documents-{{$object->_guid}} praticien-{{$object->praticien_id}} mode-collapse" style="min-width: 200px; min-height: 50px; float: left; width: 50%;">
    {{mb_include module=compteRendu template=inc_widget_documents mode=collapse modelesByOwner=$modelesByOwner.CSejour packsByOwner=$packsByOwner.CSejour}}
  </div>
{{else}}
  {{assign var=object value=$operation}}
