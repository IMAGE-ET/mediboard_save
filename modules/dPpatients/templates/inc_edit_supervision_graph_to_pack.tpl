...
<form name="edit-supervision-graph-to-pack" method="post" action="?m=dPpatients" onsubmit="return onSubmitFormAjax(this)">
  ...
  <table class="main form">
      ...
      <td>
        <select name="graph_id">
          ...
          {{foreach from=$items item=_item}}
            <option value="{{$_item->_id}}" {{if $_item->_class == $link->graph_class && $_item->_id == $link->graph_id}} selected {{/if}}>{{$_item}}</option>
          {{/foreach}}
        </select>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        {{if $link->_id}}
          <button type="button" class="trash" onclick="confirmDeletion(this.form,{typeName:'', objName:'{{$link->_view|smarty:nodefaults|JSAttribute}}'})">
            {{tr}}Delete{{/tr}}
          </button>
        {{/if}}
      </td>
