...
{{if $is_last}}
  ...
  <select name="{{$_feature}}-{{$components[0]}}" {{if $is_inherited}} disabled {{/if}} onchange="changeValuesDisplayMode('{{$_feature}}')">
    <option value="classic" {{if $value[0] == 'classic'}}selected="selected" {{/if}}>{{tr}}config-dPpatients-CConstantesMedicales-graphs_display_mode-mode-classic{{/tr}}</option>
    <option value="time" {{if $value[0] == 'time'}}selected="selected" {{/if}}>{{tr}}config-dPpatients-CConstantesMedicales-graphs_display_mode-mode-time{{/tr}}</option>
  </select>
