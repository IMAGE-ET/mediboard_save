...
class CSetupdPstock extends CSetup {
  ...
  function __construct() {
    ...
    $sql = "ALTER TABLE `product_order_item` ADD `renewal` ENUM ('0','1') NOT NULL DEFAULT '1'";
    $this->addQuery($sql);
    $sql = "ALTER TABLE `product_order_item` ADD INDEX (`renewal`)";
    $this->addQuery($sql);
    
    $this->makeRevision("1.35");
    $sql = "ALTER TABLE `product_order_item` ADD `septic` ENUM ('0','1') NOT NULL DEFAULT '0'";
    $this->addQuery($sql);
