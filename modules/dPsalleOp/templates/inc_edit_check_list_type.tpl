  ...
  <table class="main form">
    ...
    <tr>
      <td class="button" colspan="2">
        {{if $modal}}
          <button class="submit" type="button" onclick="return onSubmitFormAjax(form, Control.Modal.close);">{{tr}}Save{{/tr}}</button>
        {{else}}
  ...
  <table class="main tbl">
    ...
    {{foreach from=$list_type->_ref_categories item=_category}}
      <tr>
        ...
        <td>
          <a href="#1" onclick="CheckList.editItemCategory('{{$list_type->_id}}', '{{$_category->_id}}', '{{$callback}}')" >
    ...
    <tr>
      <td colspan="3">
        <button class="new compact" onclick="CheckList.editItemCategory('{{$list_type->_id}}', '{{$callback}}')" >
