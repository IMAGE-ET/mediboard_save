...
<form name="edit-supervision-graph-instant-data" method="post" action="?m=dPpatients" onsubmit="return onSubmitFormAjax(this)">
  ...
  <table class="main form">
    ...
    <tr>
      ...
      <td>{{mb_field object=$instant_data field=value_type_id autocomplete="true,1,50,true,true" form="edit-supervision-graph-instant-data"}}</td>
    </tr>

    <tr>
      <th>{{mb_label object=$instant_data field=value_unit_id}}</th>
      <td>{{mb_field object=$instant_data field=value_unit_id autocomplete="true,1,50,true,true" form="edit-supervision-graph-instant-data"}}</td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        {{if $instant_data->_id}}
          <button type="button" class="trash" onclick="confirmDeletion(this.form,{typeName:'', objName:'{{$instant_data->_view|smarty:nodefaults|JSAttribute}}'})">
            {{tr}}Delete{{/tr}}
          </button>
        {{/if}}
      </td>
