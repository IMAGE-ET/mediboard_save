      ...
      if(CModule::getActive("bcb")){
        ...
        foreach($produits as $_produit){
          ...
          if($produit_livret_id){
            ...
            $query .= "commentaire = '".$ds_std->escape($commentaire)."', 
                       code_ucd = '$code_ucd', 
                       code_cis = '$code_cis' 
          ...
          // Sinon, on le crée
            ...
            $query = $ds_std->prepare("INSERT INTO produit_livret_therapeutique 
                                       (`owner_crc`,`code_cip`,`code_ucd`,`code_cis`,`commentaire`) VALUES 
                                       (%1, %2, %3, %4, %5)", $owner_crc, $code_cip, $code_ucd, $code_cis, $commentaire);
          }
