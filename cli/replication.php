...
/* Enable to configure a server and to write in a XML file
 *
 */
 ...
 * @return An array containing the SQL queries to request
 */
...
/* Enable to generate a configuration file, my.cnf of just a TXT file
 * @return An array containing the two SQL queries to request
 */
...
####### Fin replication circulaire #######\n";
      ...
      switch ($method['method']) {
        case 'mysqld':
            ...
            if (preg_match("/^\s*bind-address/i", $lineContent)) {
              ...
              if (preg_match("/^\s*skip-networking/i", $lineContent)) {
                ...
                if (preg_match("/^\s*\[mysqld\]/i", $lineContent)) {
                  ...
                  if (preg_match("/^\s*\max_binlog_size/i", $lineContent)) {
          ...
          check_errs(file_put_contents($method['filename'].".".$myServer->getAttribute('ip').".new", $fileContent), false, "Unable to write ".$method['filename'].".".$myServer->getAttribute('ip').".new", $method['filename'].".".$myServer->getAttribute('ip').".new file successfully written!");
          echo "\n";
          cecho("Please, replace the original configuration file by the .new file created. Then restart MySQL, before running the SQL queries.", "red");
          break;
          
        // If we just want to generate a TXT file
        case 'file':
          check_errs(file_put_contents($method['filename'], $conf), false, "Unable to write into ".$method['filename'].".", "File ".$method['filename']." successfully written!");
          echo "\n";
          cecho("Please, replace the original configuration file by the .new file created. Then restart MySQL, before running the SQL queries.", "red");
          break;
...
FLUSH PRIVILEGES;";
      ...
      // SQL second query
      $sql2 = "SLAVE STOP;
CHANGE MASTER TO MASTER_HOST='".$myOtherServer->getAttribute('ip')."', MASTER_USER='".$myOtherServer->getElementsByTagName('esclave')->item(0)->getAttribute('username')."', MASTER_PASSWORD='".$myOtherServer->getElementsByTagName('esclave')->item(0)->getAttribute('password')."', MASTER_LOG_FILE='".$masterStatus['File']."', MASTER_LOG_POS=".$masterStatus['Position'].";
START SLAVE;";
      ...
      if ($myServer->getAttribute('first') == "y") {
        ...
        if ($answerSQL != "n") {
          ...
          // Else, SSH
            ...
            // Command via SSH
            exec("ssh ".$sshUser."@".$myServer->getAttribute('ip')." mysqldump -u ".$myServer->getAttribute('dbusername')." -P ".$myServer->getAttribute('dbport')." -p".$myServer->getAttribute('dbpassword')." --databases ".$root->getAttribute('db')." > /tmp/mysqldump.sql", $result, $returnVar);
          }
        ...
        if ($answerSQL != "n") {
          if ($myServer->getAttribute('localhost') == "y") {
            ...
            $sshUser = recup("Username allowed to connect to ".$myServer->getAttribute('ip').": ");
            // Command via SSH
            exec("ssh ".$sshUser."@".$myServer->getAttribute('ip')." mysql -u ".$myServer->getAttribute('dbusername')." -P ".$myServer->getAttribute('dbport')." -p".$myServer->getAttribute('dbpassword')." < /tmp/mysqldump.sql", $result, $returnVar);
          }
...
/* Get the two servers name
 * @return An array containing the two names
 */
...
function getServerByName($name, $file) {
  ...
  check_errs(isset($myServer), false, "Unable to find the server ".$name." in the ".$file." file.", "Server ".$name." found!");
...
function getOtherServerByName($name, $file) {
  ...
  check_errs(isset($myServer), false, "Unable to find the other server than ".$name." in the ".$file." file.", "Server ".$aServer->getAttribute('nom')." found!");
...
// Return the binlog filename and its position
...
/**
 * $text : le texte a colorer
 * $txtColor : couleur voulue (black, red, green, cyan, magenta, black...)
 * $bgColor : couleur de fond
 * $styleTxt : style du texte (bold, underline, reverse, flashing)
 *
 * return string
 * A noter que certaines mises en forme ne fonctionnent pas sous tous les clients
 **/
