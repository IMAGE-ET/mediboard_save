{{* $Id: vw_idx_admission.tpl 13361 2011-10-03 15:19:15Z mytto $ *}}

{{*
 * @package Mediboard
 * @subpackage dPadmissions
 * @version $Revision: 13361 $
...
function reloadFullAdmissions(filterFunction) {
  ...
  var url = new Url("ssr", "httpreq_vw_all_sejours");
  url.addParam("date"      , "{{$date}}");
  url.addParam("service_id", $V(oForm.service_id));
...
function reloadAdmission(filterFunction) {
  var oForm = getForm("selType");
  var url = new Url("ssr", "httpreq_vw_sejours");
...
Main.add(function () {
  var totalUpdater = new Url("ssr", "httpreq_vw_all_sejours");
  totalUpdater.addParam("date", "{{$date}}");
  totalUpdater.periodicalUpdate('allAdmissions', { frequency: 120 });
  
  var listUpdater = new Url("ssr", "httpreq_vw_sejours");
