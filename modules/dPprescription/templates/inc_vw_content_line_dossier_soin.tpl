        ...
        {{/if}}
		      ...
		      {{else}}
				  ...
				  {{assign var=_quantite value=$quantite}}
					...
					{{* Si une case du plan de soins est composée de plusieurs administrations *}}
                      ...
                      PlanSoins.addAdministration({{$line_id}}, "{{$quantite}}", "{{$unite_prise}}", "{{$line_class}}","{{$_date}} {{$_hour}}:00:00","{{$list_administrations}}","{{$planification_id}}","{{$multiple_adm}}");
                       ...
                       /{{$quantite}}
						   ...
						   {{/if}}
							 ...
							 {{if (($line->_fin_reelle && ($line->_fin_reelle|date_format:"%Y-%m-%d %H:00:00" < $_date_hour)) || $line->_debut_reel|date_format:"%Y-%m-%d %H:00:00" > $_date_hour) && $line->_active && !$line->inscription && $quantite=="-"}}
