...
{{if $dossier_medical->_count_antecedents && ($dossier_medical->_count_antecedents > $dossier_medical->_count_allergies)}}
  ...
  <table class="tbl" id="antecedents{{$sejour_id}}" style="text-align:left;  display: none;">
   <tr>
     <th class="title">Antécédents <small>({{$dossier_medical->_count_antecedents}})</small></th>
   </tr>
   {{foreach from=$antecedents key=name item=cat}}
      {{if $name != "alle" && $cat|@count}}
      <tr>
        <th>{{tr}}CAntecedent.type.{{$name}}{{/tr}}</th>
      </tr>
