...
<table class="tbl" id="plan_soin" style="display: none;">
	<tr>
	  <th class="title" colspan="100">
	  	<button type="button" class="tick" onclick="PlanSoins.applyAdministrations();" id="button_administration" style="float: right"></button>
      {{if "soins suivi hide_old_line"|conf:"CGroups-$g" && $date == $dnow}}
        {{if $hide_old_lines}}
          <button type="button" class="search" onclick="updatePlanSoinsPatients(0)" style="float: right">Afficher les prescriptions terminées ({{$hidden_lines_count}})</button>
        {{else}}
          <button type="button" class="search" onclick="updatePlanSoinsPatients(1)" style="float: right">Masquer les prescriptions terminées</button>
        {{/if}}
      {{/if}}
        ...
        {{foreach from=$_dates key=_date_reelle item=_hours}}
          {{foreach from=$_hours key=_heure_reelle item=_hour}}
            <th class="{{$_date}}-{{$moment_journee}}"
              ...
              <a href="#1" onclick="PlanSoins.selColonne('{{$_date_reelle}}-{{$_hour}}');" {{if $smarty.now|date_format:"%Y-%m-%d %H" == "$_date_reelle $_hour"}}style="background-color: #fff;"{{/if}}'>{{$_hour}}h</a>
    ...
    {{foreach from=$prescription->_ref_lines_elt_for_plan key=name_chap item=_elements_by_chap}}
        ...
        {{foreach from=$elements_cat item=_element name="foreach_cat"}}
          {{foreach from=$_element key=unite_prise item=element name="foreach_elt"}}       
            {{mb_include module="prescription" template="inc_vw_line_dossier_soin"
