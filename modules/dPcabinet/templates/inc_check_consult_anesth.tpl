<table class="tbl">
  ...
  <tr>
    <td style="white-space: pre-wrap;">Identification du médecin anesthésiste de la phase pré-anesthésique</td>
  ...
  <tr>
    ...
    {{if $conf.dPcabinet.CConsultAnesth.show_facteurs_risque &&
      ($dm_sejour->risque_antibioprophylaxie != "NR" || $dm_sejour->risque_prophylaxie != "NR" ||  $dm_patient->facteurs_risque ||
  ...
  <tr>
    ...
    {{if ($consult_anesth->mallampati && $consult_anesth->bouche && $consult_anesth->distThyro) || $consult_anesth->conclusion}}
      {{assign var="result" value=true}}
    {{/if}}
    <td style="white-space: pre-wrap;">Mention de l'évaluation des conditions d'abord des voies aériennes supérieures en phase pré-anesthésique dans le document traçant la CPA</td>
