...
{{if $conf.dPcabinet.CConsultAnesth.check_close}}
  <style>
    ...
    .atc-ok {
      background-color: #72c364 !important;
    }

    .atc-ko {
      background-color: #ff8486 !important;;
    }

    .atc-ko span, .atc-ok span {
      color:black;
...
{{if $conf.dPpatients.CAntecedent.mandatory_types}}
  <tr>
    <th colspan="2">
      Antécédents obligatoires
    </th>
  </tr>
  {{foreach from=$mandatory_types key=_type item=_atc}}
    <tr>
      <td style="text-align: right;" class="atc-{{if $_atc|@count}}ok{{else}}ko{{/if}}">
          {{tr}}CAntecedent.type.{{$_type}}{{/tr}}
      </td>
      <td>
        {{foreach from=$_atc item=__atc}}
          <span>{{$__atc}}</span>
        {{foreachelse}}
          {{assign var=need_more_atc value=1}}
          Non défini
        {{/foreach}}
      </td>
    </tr>
