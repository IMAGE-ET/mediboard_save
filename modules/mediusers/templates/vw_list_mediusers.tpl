<table class="tbl">
  ...
  <tr {{if $curr_user->_id == $object->_id}}class="selected"{{/if}}>
    ...
    {{if $curr_user->_ref_user->_id}}
     ...
     <td class="text"><a href="#" onmouseover="ObjectTooltip.createEx(this,'{{$curr_user->_guid}}', 'identifiers')"
         							 onclick="showMediuser('{{$user_id}}')" class="mediuser" style="border-left-color: #{{$curr_user->_ref_function->color}};">
         							{{mb_value object=$curr_user field=_user_username}}</a></td>
